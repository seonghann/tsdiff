...
if __name__ == "__main__":
    ...
    try:
        ...
        for it in range(start_iter, config.train.max_iters + 1):
            loss, loss_g, loss_l, lr_g, lr_l, grad_norm, n, n_g, n_l = train(it)
            # print(loss/n)
            loss_sum += loss
            loss_g_sum += loss_g
            loss_l_sum += loss_l
            ...
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                    ...
                    wandb.log(
                        {
                            ...
                            "train/loss_global": loss_g_sum / n_g_sum,
                            "train/loss_local": loss_l_sum / n_l_sum,
                            "train/lr_global": lr_g,
                            "train/lr_local": lr_l,
                ...
                #        batch = torch.zeros(sample.num_nodes,dtype=torch.long).to(args.device)
                #        pos_gen, pos_gen_traj = model.langevin_dynamics_sample(
                #                atom_type=sample.atom_type,
                #                r_feat=sample.r_feat,
                #                p_feat=sample.p_feat,
                #                pos_init=pos_init,
                ...
                #        dm_gen = (pos_gen.unsqueeze(0) - pos_gen.unsqueeze(1)).norm(dim=-1)
