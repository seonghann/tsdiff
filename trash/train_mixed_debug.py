...
if __name__ == "__main__":
    ...
    # Optimizer
    optimizer = get_optimizer(config.train.optimizer, model.model_params)
    ...
    if resume:
        ...
        optimizer.load_state_dict(ckpt["optimizer_global"])
        scheduler.load_state_dict(ckpt["scheduler_global"])
    ...
    with open("data/TS/b97d3/debug_test.pkl", "rb") as f:
    ...
    def validate(it):
        ...
        print("[Validate] Iter %05d | Loss %.6f" % (it, avg_loss))
        if use_wandb:
            wandb.log(
                {
                    "val/loss": avg_loss,
        ...
        for it in range(start_iter, config.train.max_iters + 1):
            ...
            if it % config.train.log_freq == 0:
                print(
                    ...
                    % (it, loss_sum / n_sum, grad_norm_sum / config.train.val_freq, lr)
                )
                loss_sum = 0
                n_sum = 0
                grad_norm_sum = 0
            if it % config.train.val_freq == 0 or it == config.train.max_iters:
                ...
                #        'optimizer_global': optimizer.state_dict(),
                #        'scheduler_global': scheduler.state_dict(),
                #        'iteration': it,
                ...
                with torch.no_grad():
                    ...
                    for sample in test_data[:2]:
                        ...
                        pos_init = torch.randn(sample.num_nodes, 3).to(args.device)
                        batch = torch.zeros(sample.num_nodes, dtype=torch.long).to(
                            args.device
                        )
                        pos_gen, pos_gen_traj = model.langevin_dynamics_sample(
                            ...
                            clip=1000,
                            clip_local=None,
                            sampling_type="ddpm_det",
                            eta=1.0,
                        )
                        pos_gen = pos_gen.cpu()

                        pos = sample.pos.cpu()
                        dm = (pos.unsqueeze(0) - pos.unsqueeze(1)).norm(dim=-1)
                        dm_gen = (pos_gen.unsqueeze(0) - pos_gen.unsqueeze(1)).norm(
                            dim=-1
                        )
                        rmsd = torch.sqrt((torch.abs(dm - dm_gen)).mean())

                        crmsd = torch.sqrt((((pos - pos_gen) ** 2).sum(-1)).mean())
                        print(rmsd, crmsd)
