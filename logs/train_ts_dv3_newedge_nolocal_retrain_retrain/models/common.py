...
def extend_ts_graph_order_radius(num_nodes, pos, edge_index, edge_type, batch, order=3, cutoff=10.0, 
                              ):
...
def _extend_graph_order(num_nodes, edge_index, edge_type, order=3):
    ...
    type_highorder = torch.where(adj_order > 1, num_types**2 + adj_order - 1, torch.zeros_like(adj_order))
