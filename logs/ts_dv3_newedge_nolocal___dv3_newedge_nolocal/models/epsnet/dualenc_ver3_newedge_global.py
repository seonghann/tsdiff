...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def forward(self, atom_type, r_feat, p_feat, pos, bond_index, bond_type, batch, time_step, return_edges=True, **kwargs):
        """
        Args:
        ...
        # local edge mask는 
        # 1) edge_index_local 에 들어있으면서,
        # 2) edge_type이 r,p에서 모두 같은경우만을 의미한다.
        local_edge_mask = calc_local_edge_mask(edge_index_global, edge_index_local)
        local_edge_mask *= edge_type_global_r == edge_type_global_p
        ...
        edge_attr_r = self.edge_encoder_global(
                ...
                edge_type=edge_type_global_r
                )   # Embed edges
        edge_attr_p = self.edge_encoder_global(
                edge_length=edge_length_global, 
                edge_type=edge_type_global_p
                )
        ...
        h_pair_local = assemble_atom_pair_feature(
            ...
            edge_index=edge_index_local[:,edge_type_r==edge_type_p],
            edge_attr=edge_attr_local[edge_type_r==edge_type_p],
        )    # (E_local, 2H)
        ## Invariant features of edges (bond graph, local)
        edge_inv_local = self.grad_local_dist_mlp(h_pair_local) * (1.0 / sigma_edge) # (E_local, 1)

        if return_edges:
            return edge_inv_global, edge_inv_local, edge_index_global, edge_type_global, edge_length_global, local_edge_mask
        else:
