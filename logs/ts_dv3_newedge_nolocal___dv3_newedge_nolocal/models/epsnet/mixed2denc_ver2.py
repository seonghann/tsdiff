...
class Mixed2DEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.atom_emb = torch.nn.Embedding(30, config.hidden_dim)
        ...
        '''
        self.model_params = nn.ModuleList([self.atom_emb, self.bond_emb, self.graph_encoder, self.grad_dist_mlp])
        # denoising diffusion
        ## betas
    ...
    def forward(self, atom_type, pos, bond_index, bond_type, batch, time_step, 
        ...
        #h_pair = node_pair
        h_pair = torch.cat([
            node_attr[edge_index_global[0]],
            node_attr[edge_index_global[1]]
            ], dim=-1)
        ## Assemb#le pairwise features
    ...
    def get_loss(self, atom_type, pos, bond_index, bond_type, batch, 
        ...
        return self.get_loss_diffusion(atom_type, pos, bond_index, bond_type, 
                                       batch, num_nodes_per_graph, num_graphs, 
                                       anneal_power)

    def get_loss_diffusion(self, atom_type, pos, bond_index, bond_type, batch, 
                           num_nodes_per_graph, num_graphs, anneal_power=2.0,):
        ...
        #loss = eq_transform(d_target - edge_inv, pos_perturbed, edge_index, edge_length)
        loss = torch.sum(loss, dim=-1, keepdim=True)
        return loss


    def langevin_dynamics_sample(
            self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, 
        ...
        return self.langevin_dynamics_sample_diffusion(
                atom_type, pos_init, bond_index, bond_type, batch, num_graphs, 
