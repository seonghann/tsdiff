...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def forward(self, atom_type, r_feat, p_feat, pos, bond_index, bond_type, batch, time_step, 
        ...
        if hasattr(self, "TS") and self.TS == True:
            nb = self.num_bond_types
            mask = edge_type//nb**2==0
            high_order_type = torch.where(~mask, edge_type%nb**2 + nb, 0)
            edge_type1 = torch.where(mask, edge_type//nb, 0) + high_order_type
            edge_type2 = torch.where(mask, edge_type%nb, 0) + high_order_type
            edge_attr1 = self.edge_encoder_global(edge_length=edge_length, edge_type=edge_type1)   # Embed edges
            edge_attr2 = self.edge_encoder_global(edge_length=edge_length, edge_type=edge_type2)
            edge_attr_global = self.edge_cat_global(torch.cat([edge_attr1, edge_attr2], -1))
        else:
            nb = self.num_bond_types
            mask = edge_type//nb**2==0
            high_order_type = torch.where(~mask, edge_type%nb**2 + nb, 0)
            edge_type1 = torch.where(mask, edge_type%nb, 0) + high_order_type
            edge_attr_global = self.edge_encoder_global(edge_length=edge_length, edge_type=edge_type1)   # Embed edges
        # edge_attr += temb_edge
        ...
        if hasattr(self, "TS") and self.TS == True:
            ...
            edge_type1 = torch.where(mask, edge_type//nb, 0) + high_order_type
            edge_type2 = torch.where(mask, edge_type%nb, 0) + high_order_type
            edge_attr1 = self.edge_encoder_local(edge_length=edge_length, edge_type=edge_type1)   # Embed edges
            edge_attr2 = self.edge_encoder_local(edge_length=edge_length, edge_type=edge_type2)
            edge_attr_local = self.edge_cat_local(torch.cat([edge_attr1, edge_attr2], -1))
        else:
            nb = self.num_bond_types
            mask = edge_type//nb**2==0
            high_order_type = torch.where(~mask, edge_type%nb**2 + nb, 0)
            edge_type1 = torch.where(mask, edge_type%nb, 0) + high_order_type
            edge_attr_local = self.edge_encoder_local(edge_length=edge_length, edge_type=edge_type1)   # Embed edges
        # edge_attr += temb_edge
    ...
    def get_loss(self, atom_type, r_feat, p_feat, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
        ...
        if self.model_type == 'diffusion':
            return self.get_loss_diffusion(atom_type, r_feat, p_feat, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
                anneal_power, return_unreduced_loss, return_unreduced_edge_loss, extend_order, extend_radius, is_sidechain)
        elif self.model_type == 'dsm':
            return self.get_loss_dsm(atom_type, r_feat, p_feat, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
                anneal_power, return_unreduced_loss, return_unreduced_edge_loss, extend_order, extend_radius, is_sidechain)


    def get_loss_diffusion(self, atom_type, r_feat, p_feat, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
                 anneal_power=2.0, return_unreduced_loss=False, return_unreduced_edge_loss=False, extend_order=True, extend_radius=True, is_sidechain=None):
        N = atom_type.size(0)
        node2graph = batch
        ...
        aa, bb = 2, 5
        loss = (aa*loss_global + bb*loss_local)/(aa+bb)
    ...
    def langevin_dynamics_sample(self, atom_type, r_feat, p_feat, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
        ...
        if self.model_type == 'diffusion':
            return self.langevin_dynamics_sample_diffusion(atom_type, r_feat, p_feat, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius, 
            ...
            return self.langevin_dynamics_sample_dsm(atom_type, r_feat, p_feat, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius, 
                        n_steps, step_lr, clip, clip_local, clip_pos, min_sigma, is_sidechain,
                        global_start_sigma, w_global, w_reg)


    def langevin_dynamics_sample_diffusion(self, atom_type, r_feat, p_feat, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
                                 n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None, min_sigma=0, is_sidechain=None,
                                 global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, **kwargs):
        ...
        with torch.no_grad():
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                ...
                if sampling_type == 'generalized' or sampling_type == 'ddpm_noisy' or sampling_type == 'ddpm_det':
                    b = self.betas
                    t = t[0]
