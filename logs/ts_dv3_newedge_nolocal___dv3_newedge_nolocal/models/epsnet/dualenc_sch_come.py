...
from ..encoder import SchNetEncoder, GINEncoder, ComENet, get_edge_encoder, TwoLayerLinear
...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        """
        self.atom_emb = nn.Embedding(100, config.hidden_dim, max_norm=10.0)
        ...
        self.lin_feature1 = TwoLayerLinear(
            config.num_radial * config.num_spherical**2, 
            config.hidden_dim, 
            config.hidden_dim, 
            config.dropout,
        )
        ...
        self.lin_feature2 = TwoLayerLinear(
            config.num_radial * config.num_spherical, 
            config.hidden_dim, 
            config.hidden_dim, 
            config.dropout
        )

        self.encoder_local = ComENet(
            cutoff=config.cutoff,
            num_layers=config.num_convs_local,
            hidden_channels=config.hidden_dim,
            out_channels=config.hidden_dim,   
            num_radial=config.num_radial,
            num_spherical=config.num_spherical,
            num_output_layers=3,
            lin_feature=[self.lin_feature1, self.lin_feature2]
            )

        self.edge_cat = torch.nn.Sequential(
                torch.nn.Linear(self.edge_encoder_global.out_channels*3, self.edge_encoder_global.out_channels),
                activation_loader(config.edge_cat_act),
                torch.nn.Linear(self.edge_encoder_global.out_channels, self.edge_encoder_global.out_channels)
                )
        """
        ...
        if self.TS:
            self.edge_cat_global = torch.nn.Sequential(
                    torch.nn.Linear(self.edge_encoder_global.out_channels*2, self.edge_encoder_global.out_channels),
                    activation_loader(config.edge_cat_act),
                    torch.nn.Linear(self.edge_encoder_global.out_channels, self.edge_encoder_global.out_channels)
                    )
    ...
    def edge_embedding(self, edge_type, edge_length, encoder_type="global"):
        edge_encoder = getattr(self, f"edge_encoder_{encoder_type}")
        if hasattr(self, "TS") and self.TS == True:
            edge_cat = getattr(self, f"edge_cat_{encoder_type}")
            high_order_type = edge_type // (self.num_bond_types**2)
            edge_type1 = (edge_type // self.num_bond_types) % self.num_bond_types + high_order_type
            edge_type2 = edge_type % self.num_bond_types + high_order_type
            is_changed = edge_type1 != edge_type2
           
            edge_attr = torch.zeros(edge_type1.size(0), edge_encoder.out_channels, device=edge_type.device)
            edge_attr_ = edge_encoder(
                    edge_length=edge_length[~is_changed], 
                    edge_type=edge_type1[~is_changed]
                    )
            edge_attr[~is_changed] = edge_attr_
            
            edge_type1 = edge_type1[is_changed]
            edge_type2 = edge_type2[is_changed]

            edge_attr1 = edge_encoder(
                    edge_length=edge_length[is_changed], 
                    edge_type=edge_type1
                    )   # Embed edges
            edge_attr2 = edge_encoder(
                    edge_length=edge_length[is_changed], 
                    edge_type=edge_type2
                    )
            edge_attr_ = edge_cat(torch.cat([edge_attr1, edge_attr2], -1))
            edge_attr[is_changed] = edge_attr_

        else:
            high_order_type = edge_type // (self.num_bond_types**2)
            edge_type1 = (edge_type % self.num_bond_types**2) + high_order_type
            edge_attr = edge_encoder(edge_length=edge_length, edge_type=edge_type1)   # Embed edges
        
        return edge_attr

    def pair_embedding(self, batch, atom_type, pos, edge_index, edge_length, edge_attr, encoder_type="global"):
        encoder = getattr(self, f"encoder_{encoder_type}")
        out = encoder(
                z=atom_type,
                pos=pos,
                batch=batch,
        ...
        if encoder_type == "global":
            ...
            node, edge_attrs = out
            edge_attr = [edge_attr] + edge_attrs
            edge_attr = self.edge_cat(torch.cat(edge_attr, dim=-1))
        pair = assemble_atom_pair_feature(
                node_attr=node,
                edge_index=edge_index,
                edge_attr=edge_attr
                )
    ...
    def forward(self, atom_type, pos, bond_index, bond_type, batch, time_step, 
        ...
        # node_embedding
        atom_type = self.atom_emb(atom_type)

        # Encoding global
        edge_global = self.edge_embedding(edge_type, edge_length, encoder_type="global")
        pair_global = self.pair_embedding(batch, atom_type, pos, edge_index, edge_length, edge_global, encoder_type="global")
        edge_inv_global = self.grad_global_dist_mlp(pair_global) * (1.0 / sigma_edge)    # (E_global, 1)
        ## Invariant features of edges (radius graph, global)
        
        # Encoding local
        edge_local = self.edge_embedding(edge_type[local_edge_mask], edge_length[local_edge_mask], encoder_type="local")
        edge_index_local = edge_index[:, local_edge_mask]
        pair_local = self.pair_embedding(batch, atom_type, pos, edge_index_local, None, edge_local, encoder_type="local")
        ## Invariant features of edges (bond graph, local)
        if isinstance(sigma_edge, torch.Tensor):
            edge_inv_local = self.grad_local_dist_mlp(pair_local) * (1.0 / sigma_edge[local_edge_mask]) # (E_local, 1)
        else:
            edge_inv_local = self.grad_local_dist_mlp(pair_local) * (1.0 / sigma_edge) # (E_local, 1)
