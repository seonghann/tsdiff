...
class DualEncoderEpsNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.edge_cat = torch.nn.Sequential(
            torch.nn.Linear(
                self.edge_encoder_global.out_channels * 3,
    ...
    def edge_embedding(self, edge_type, edge_length, encoder_type="global"):
        ...
        if hasattr(self, "TS") and self.TS == True:
            ...
            ) % self.num_bond_types + high_order_type
            edge_type2 = edge_type % self.num_bond_types + high_order_type
            is_changed = edge_type1 != edge_type2

            edge_attr = torch.zeros(
                edge_type1.size(0), edge_encoder.out_channels, device=edge_type.device
            )
            edge_attr_ = edge_encoder(
                edge_length=edge_length[~is_changed], edge_type=edge_type1[~is_changed]
            )
            ...
            edge_attr1 = edge_encoder(
                edge_length=edge_length[is_changed], edge_type=edge_type1
            )  # Embed edges
            edge_attr2 = edge_encoder(
                edge_length=edge_length[is_changed], edge_type=edge_type2
            )
            ...
            edge_attr = edge_encoder(
                edge_length=edge_length, edge_type=edge_type1
            )  # Embed edges
    ...
    def pair_embedding(
        ...
        pair = assemble_atom_pair_feature(
            node_attr=node, edge_index=edge_index, edge_attr=edge_attr
        )
        return pair
    ...
    def forward(
        ...
        pair_global = self.pair_embedding(
            batch,
            atom_type,
            pos,
            edge_index,
        ...
        edge_inv_global = self.grad_global_dist_mlp(pair_global) * (
            1.0 / sigma_edge
        )  # (E_global, 1)
        ## Invariant features of edges (radius graph, global)

        # Encoding local
        edge_local = self.edge_embedding(
            edge_type[local_edge_mask],
            edge_length[local_edge_mask],
            encoder_type="local",
        )
        edge_index_local = edge_index[:, local_edge_mask]
        pair_local = self.pair_embedding(
            batch,
            atom_type,
            pos,
            edge_index_local,
        ...
        if isinstance(sigma_edge, torch.Tensor):
            edge_inv_local = self.grad_local_dist_mlp(pair_local) * (
    ...
    def get_loss_dsm(
        ...
        if self.config.edge_encoder == "gaussian":
            ...
            d_perturbed = torch.clamp(d_perturbed * d_sgn, min=0.01, max=float("inf"))
        d_target = (
            1.0 / (sigmas_edge**2) * (d_gt - d_perturbed)
        )  # (E_global, 1), denoising direction
        ...
        loss_global = (
            0.5
            * ((node_eq_global - target_pos_global) ** 2)
            * (sigmas_pos**anneal_power)
        )
        loss_global = 2 * torch.sum(loss_global, dim=-1, keepdim=True)
        ...
        loss_local = (
            0.5
            * ((node_eq_local - target_pos_local) ** 2)
            * (sigmas_pos**anneal_power)
        )
        loss_local = 5 * torch.sum(loss_local, dim=-1, keepdim=True)
