...
if __name__=="__main__":
    ...
    def func(x):
        ...
        out = []
        for i in range(n):
            #r_noised, p_noised = add_noise_to_distance_lst(
            #        r_pos, p_pos, sigma=sigma, steps=steps, patience=patience, device=device
            #        )
            r_noised, p_noised, _ = add_noise_to_distance_qst(
                    r_pos, p_pos, ts_pos, sigma=sigma, steps=steps, patience=patience, device=device
                    )
            ...
            out.append((r_noised, p_noised, ts_noised))
        return out


    if not os.path.isdir(f"result_B/sigma_{sigma:0.1e}"):
        os.system(f"mkdir result_B/sigma_{sigma:0.1e}")
    ...
    ts_aug_blocks = [[o[i][2] for o in ret] for i in range(n)]
    for i, (r, p, ts) in enumerate(zip(r_aug_blocks, p_aug_blocks, ts_aug_blocks)):
        r_msg = "\n$$$$$\n".join(r) + "\n$$$$$"
        p_msg = "\n$$$$$\n".join(p) + "\n$$$$$"
        ts_msg = "\n$$$$$\n".join(ts) + "\n$$$$$"
        with open(f"result_B/sigma_{sigma:0.1e}/b97d3_r_test_B_aug_{i}.xyz","w") as f:
            f.write(r_msg)
        with open(f"result_B/sigma_{sigma:0.1e}/b97d3_p_test_B_aug_{i}.xyz","w") as f:
            f.write(p_msg)
        with open(f"result_B/sigma_{sigma:0.1e}/b97d3_ts_test_B_aug_{i}.xyz","w") as f:
            f.write(ts_msg)
