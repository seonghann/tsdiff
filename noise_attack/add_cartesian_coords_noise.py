...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--random", "-random", default=True, type=str2bool, help="Deprecated"
    )
    parser.add_argument("--n", "-n", default=1, type=int, help="Deprecated")
    parser.add_argument("--steps", "-steps", default=3000, type=int, help="Deprecated")
    parser.add_argument(
        "--patience", "-patience", default=300, type=int, help="Deprecated"
    )
    ...
    if args.save_type == "pkl":
        ...
        for i, aug_blocks in enumerate(noised_data_iter):
            for xyz_r, xyz_p, xyz_gt, xyz_aug in zip(
                r_blocks, p_blocks, ts_blocks, aug_blocks
            ):
                r = xyz_r.split("\n")[1].strip()
                p = xyz_p.split("\n")[1].strip()
            ...
            for data in data_list:
                ...
                for feat, n_cls in zip(feats, num_cls):
                    feat_onehot.append(
                        torch.nn.functional.one_hot(feat, num_classes=n_cls)
                    )
