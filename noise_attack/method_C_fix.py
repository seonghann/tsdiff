...
if __name__=="__main__":
    ...
    def func(x):
        ...
        r_noised, p_noised  = add_noise_to_distance_lst(
                r_pos, p_pos, sigma, steps=steps, patience=patience, device=device
                )
        r_noised = reform_xyz_block(r_block, r_noised)
        p_noised = reform_xyz_block(p_block, p_noised)
        out = (r_noised, p_noised)
        return out

    if not os.path.isdir(f"result_C_fix/sigma_{sigma:0.1e}"):
        os.system(f"mkdir result_C_fix/sigma_{sigma:0.1e}")

    with mp.Pool(16) as p:
        ret = p.map(func, list(zip(r_blocks, p_blocks))[:100])
    #ret = []
    ...
    r_msg = "\n$$$$$\n".join(r_aug_blocks) + "\n$$$$$"
    p_msg = "\n$$$$$\n".join(p_aug_blocks) + "\n$$$$$"
    with open(f"result_C_fix/sigma_{sigma:0.1e}/b97d3_r_test_B.xyz","w") as f:
        f.write(r_msg)
    with open(f"result_C_fix/sigma_{sigma:0.1e}/b97d3_p_test_B.xyz","w") as f:
        f.write(p_msg)
