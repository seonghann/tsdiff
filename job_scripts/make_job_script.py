def create_msg(
    job_name, node, config_file_path, ppn=4, run_file_path="train.py", **kwargs
):
    msg = f"""#!/bin/bash
#PBS -N {job_name}
...
if __name__ == "__main__":
    ...
    for cfg_fn, jn, node, kwargs in zip(
        config_file_list, job_name_list, nodes, kwargs_list
    ):
        msg = create_msg(jn, node, cfg_fn, **kwargs)
        with open(f"{jn}.sh", "w") as f:
            f.write(msg)
